...
function ico(name){
  const m={
    ...
    bed:'▣',bath:'♨',car:'▰',owner:'👤',broker:'♟',save:'💾',
    edit:'✎',delete:'🗑',archive:'▣',share:'⇪',search:'⌕',image:'▧',
    settings:'⚙',back:'‹',phone:'☎',mail:'✉',service:'✦',pdf:'▣',add:'＋'
  };
  ...
  render(`
    ...
    <section class="section">
      ...
      <div class="imageUploader">
        <label class="uploadBox">＋ إضافة صور
          <input type="file" multiple accept="image/*" onchange="handleImages(event)">
        </label>
        <label class="uploadBox">＋ إضافة فيديو
          <input type="file" accept="video/*" onchange="handleVideo(event)">
        </label>
      </div>
      <div id="previews" class="previewGrid"></div>
      <p class="smallNote">الصور تظهر مصغرة هنا ولا تكبر على صفحة الإدخال. الصورة الأولى تعتبر الرئيسية.</p>
    </section>
    ...
    <section class="section">
      ...
      <div class="gridForm">
        ...
        <div class="field"><label>رقم العرض</label><input id="offerNo" value="${esc(offerValue)}"></div>
        <div class="field"><label>نوع العقار</label><input id="type" value="${esc(p?.type)}"></div>
        <div class="field"><label>التصنيف: سكني/تجاري</label><input id="category" value="${esc(p?.category)}"></div>
    ...
    <div class="fixedBar noPrint">
      <button class="primary" onclick="saveProperty('details')">${ico('save')} حفظ العقار</button>
...
function handleImages(e){
  ...
  files.forEach(file=>{
    ...
    r.onload=()=>{
      ...
      if(done===files.length) renderPreviews();
    };
...
function collect(){
  ...
  return {
    id: editingId || uid(),
    offerNo: val('offerNo') || nextOfferNo(),
...
function saveProperty(next='details'){
  try{
    ...
    if(next==='new'){
      editingId=null;
      selectedImages=[];
      setTimeout(()=>renderForm(),350);
    }else{
      setTimeout(()=>renderDetails(data.id),350);
    }
  }catch(err){
...
function renderDetails(id){
  ...
  render(`
    ...
    <section class="titleBlock">
      ...
      ${p.mapLink?`<button class="contactLink" onclick="openMap('${esc(p.mapLink)}')">${ico('map')} فتح الموقع في الخرائط</button>`:''}
    </section>
    ...
    <section class="section">
      ...
      <div class="servicesGrid">${(p.services||[]).map(s=>`<div class="serviceItem">${ico('service')}${esc(s)}</div>`).join('')||'-'}</div>
    </section>
...
function relatedLink(name,key){
  if(!name)return '-';
  return `<a class="contactLink" href="#" onclick="showRelated('${esc(name)}','${key}');return false;">${esc(name)}</a>`;
}
...
function openPdf(p,preview=false){
  ...
  <head>
    ...
    <style>
      ...
      body{margin:0;background:#eee;font-family:Arial,Tahoma,sans-serif;color:#123}
      .page{width:210mm;height:297mm;background:#fff;margin:0 auto;position:relative;padding:12mm 12mm 25mm;box-sizing:border-box;page-break-after:always;overflow:hidden}
      .head{height:24mm;display:flex;align-items:center;justify-content:space-between;border-bottom:3px solid #004d3d;padding-bottom:6px;box-sizing:border-box}
      .logo{width:78px;max-height:64px;object-fit:contain}
      .title{text-align:center}
      .title h1{margin:0;color:#004d3d;font-size:24px}
      .title p{margin:4px 0;color:#52736b}
      .offerBox{background:#004d3d;color:#fff;border-radius:8px;padding:8px 14px;text-align:center;font-size:15px}
      .offerBox b{font-size:25px}
      .hero{display:grid;grid-template-columns:1fr 1.05fr;gap:10px;margin-top:10px}
      .heroImg{height:205px;border-radius:10px;overflow:hidden;background:#eef2f1}
      .heroImg img{width:100%;height:100%;object-fit:cover}
      .pdfGrid{display:grid;grid-template-columns:repeat(3,1fr);border:1px solid #d8e2df;border-radius:9px;overflow:hidden}
      .pdfCell{min-height:55px;border:1px solid #d8e2df;text-align:center;padding:7px}
      .pdfCell small{display:block;color:#567;font-size:11px}
      .pdfCell b{display:block;margin-top:5px;color:#0a332d;font-size:13px}
      .bar{background:#004d3d;color:#fff;text-align:center;font-weight:bold;padding:6px;border-radius:6px 6px 0 0;margin-top:10px}
      .tbl{width:100%;border-collapse:collapse}
      .tbl th{background:#00664f;color:#fff}
      .tbl td,.tbl th{border:1px solid #d8e2df;padding:6px;text-align:center;font-size:12px}
      .two{display:grid;grid-template-columns:1fr 1.15fr;gap:10px}
      .box{border:1px solid #d8e2df;border-radius:0 0 8px 8px;padding:9px;min-height:62px;font-size:12px}
      .services{display:grid;grid-template-columns:repeat(4,1fr)}
      .services div{border:1px solid #d8e2df;text-align:center;padding:8px;font-size:11px}
      .gallery{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:repeat(3,1fr);gap:10px;margin-top:14px;height:210mm}
      .gallery img{width:100%;height:100%;object-fit:cover;border-radius:8px;border:1px solid #d8e2df}
      .mapBox{border:1px solid #d8e2df;border-radius:8px;padding:10px;text-align:center;margin-top:8px}
      .mapBox a{color:#004d3d;font-weight:bold;text-decoration:none}
      .foot{position:absolute;left:0;right:0;bottom:0;background:#004d3d;color:#fff;height:18mm;display:flex;align-items:center;justify-content:space-around;font-size:11px}
      .wm{position:absolute;inset:40mm 20mm auto;opacity:.035;text-align:center;font-size:150px;font-weight:bold;color:#004d3d;z-index:0}
      .content{position:relative;z-index:1}
      .printBtn{position:fixed;top:10px;left:10px;z-index:99;padding:12px 18px;border:0;border-radius:10px;background:#004d3d;color:white}
      .closeBtn{position:fixed;top:10px;right:10px;z-index:99;padding:12px 18px;border:0;border-radius:10px;background:#777;color:white}
      @media print{body{background:#fff}.printBtn,.closeBtn{display:none}.page{margin:0}}
    </style>
  </head>
  <body>
    <button class="closeBtn" onclick="window.close()">إغلاق المعاينة</button>
    <button class="printBtn" onclick="window.print()">طباعة / حفظ PDF</button>
    ${pdfPage(p,imgs)}
  ...
  return `
    <div class="head">
      ...
      <div>
        ...
        <p style="margin:0;text-align:center;font-weight:bold">${esc(settings.company)}</p>
      </div>
    </div>
  `;
}
      ...
      <div class="content">
        ...
        <div class="two">
          ...
          <div>
            ...
            <div class="services">${(p.services||[]).map(s=>`<div>✓ ${esc(s)}</div>`).join('')||'<div>-</div>'}</div>

            ${p.mapLink?`<div class="mapBox"><a href="${mapUrl(p.mapLink)}" target="_blank">فتح الموقع في الخرائط</a></div>`:''}
          </div>
        </div>
      </div>
      ${pdfFooter()}
  ...
  render(`
    ...
    <section class="section">
      ...
      <div class="settingsGrid">
        ...
        <label class="uploadBox">استيراد نسخة احتياطية
          <input type="file" accept=".json,.aqarbackup" onchange="importBackup(event)">
        </label>
        <p class="smallNote">النسخة تحفظ العقارات والصور والشعار والإعدادات محليًا. لا ترفع بياناتك إلى GitHub.</p>
      </div>
    </section>
  `);
}
